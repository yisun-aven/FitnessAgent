...
struct LoginView: View {
    ...
    var body: some View {
        VStack(spacing: 16) {
            Text("FitnessAgent").font(.largeTitle).bold()
            ...
            if let info = auth.infoMessage { Text(info).foregroundStyle(.green).font(.footnote).multilineTextAlignment(.center) }
            if let err = auth.errorMessage { Text(err).foregroundStyle(.red).font(.footnote).multilineTextAlignment(.center) }
            ...
            Button(isSignup ? "Have an account? Sign in" : "New here? Sign up") { isSignup.toggle() }
                .font(.footnote)
        }
        .padding()
